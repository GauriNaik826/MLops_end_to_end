            ...
            for vec_name, vectorizer in VECTORIZERS.items():
                with mlflow.start_run(run_name=f"{algo_name} with {vec_name}", nested=True) as child_run:
                    try:
                        ...
                        # (2) save model locally (save_model DOES support input_example + signature)
                        with TemporaryDirectory() as tmpdir:
                            model_dir = f"{tmpdir}/sk_model_{algo_name}_{vec_name}"
                            ...
                            # (3) upload that directory as artifacts into the ACTIVE RUN (child run)
